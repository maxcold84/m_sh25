  ...
  var Cart = {
          ...
          if (carts.items.length > 0) {
            ...
            if (localCartId) {
              ...
              try {
                ...
                localStorage.setItem("cart_id", newCart2.id);
                return newCart2;
              }
            }
    ...
    async getItems() {
      ...
      try {
        ...
        const isKorean2 = document.documentElement.lang === "ko" || window.location.pathname.includes("/korean/");
        const processedItems = [];
        for (const item of records) {
          let displayPrice = item.price;
          if (isKorean2 && displayPrice < 1e3) {
            displayPrice = displayPrice * 1e3;
          } else if (!isKorean2 && displayPrice > 1e3) {
            displayPrice = displayPrice / 1e3;
          }
    ...
    async renderCart() {
      ...
      const total = items.reduce((sum, item) => {
        ...
        if (isKorean2 && price < 1e3) {
          price = price * 1e3;
        } else if (!isKorean2 && price > 1e3) {
          price = price / 1e3;
        }
