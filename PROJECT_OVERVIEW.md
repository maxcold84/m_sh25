...
- `public/`
  - 과거 Hugo 기본 빌드 산출물
  - 현재 운영 기준에서는 사용하지 않는 임시 출력 경로
- `server/`
  - Hugo 배포 산출물
  - `publishDir = "server"` 기준으로 생성되는 정적 결과물
...
- Hugo 설정의 공식 배포 출력 경로는 `server/`
- `public/`는 과거 기본 출력 경로이므로 배포 기준으로 사용하지 않음
- 원본 수정은 `assets/`, `layouts/`, `content/`, `static/`에서 하고 `server/`는 빌드 결과로 갱신

즉 이 저장소는 생성물까지 함께 관리하는 방식이며, 배포 기준 산출물은 `server/`로 통일되어 있다.
